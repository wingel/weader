...
import com.mfavez.android.feedgoal.FeedException;
import com.mfavez.android.feedgoal.R;
...
public class DbFeedAdapter {
	...
	private static class DbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            try {
                ...
                SharedPreferencesHelper.setPrefStartupDialogOnInstall(mDbfa.mCtx, true);
        ...
        private List<Feed> getOPMLResourceFeeds() throws XmlPullParserException, MalformedURLException, IOException {
            ...
            while (eventType != XmlResourceParser.END_DOCUMENT) {
                if(eventType == XmlResourceParser.START_TAG) {
                    ...
                    if (tagName.equals("outline") && parser.getAttributeCount() >= 4) {
                        feed = new Feed();
    ...
    public void cleanDbItems(long feedId) {
		...
		cleanNbr = items.size() - SharedPreferencesHelper.getPrefMaxItems(mCtx);
		if (cleanNbr > 0) {
    ...
    public Feed getFirstFeed() {
		...
		while (!cursor.isAfterLast() && !found) {
			firstFeed = getFeed(cursor.getLong(cursor.getColumnIndex(DbSchema.FeedSchema._ID)));
			cursor.moveToNext();
			found = true;
		}
